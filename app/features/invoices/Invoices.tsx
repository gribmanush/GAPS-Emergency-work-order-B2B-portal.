 ...
 * Jira: TJ-109 Firestore invoice workflow; TJ-110 Coupa-ready finance exports.
 */
"use client";
...
export function Invoices({
  ...
  invoices: Invoice[];
  role: Role;
  persistence: "firestore" | "demo" | "connecting" | "error";
  busyId: string | null;
  ...
  return <>
    ...
    <div className={`notice-banner ${persistence === "error" ? "notice-error" : ""}`}>
      <b>{persistence === "firestore" ? "Firestore connected" : persistence === "connecting" ? "Connecting to Firestore" : persistence === "error" ? "Firestore unavailable" : "Demonstration mode"}</b>
      <span>{persistence === "firestore" ? "Invoice records, approvals and export history are persisted in Firebase." : "Add the Firebase environment configuration to enable shared cloud persistence. The workflow remains available locally for demonstration."}</span>
    </div>
    ...
    <div className="table-card">
      ...
      {!rows.length ? <div className="empty"><b>No invoices in this view</b><span>Choose another status filter.</span></div> : null}
    </div>
    ...
    {selected ? <section className="panel invoice-review" aria-label={`Invoice ${selected.id} details`}>
      ...
      {canReview && ["Submitted", "Under Review"].includes(selected.status) ? <div className="review-box">
        <label>Finance review comment<textarea value={comment} onChange={event => setComment(event.target.value)} placeholder="Record the approval note or a clear rejection reason" /></label>
