 ...
 * Tax-invoice capture extended for the Firestore workflow in TJ-109.
 */
"use client";
...
export function InvoiceForm({ orders, invoices, practice }: { orders: WorkOrder[]; invoices: Invoice[]; practice: string }) {
  ...
  const eligibleOrders = useMemo(() => orders.filter(order => {
    const correctPractice = order.practice === practice;
  ...
  return <div className="form-grid">
    ...
    <label className="full">Supporting tax invoice<input name="attachment" type="file" accept=".pdf,.png,.jpg,.jpeg" required /><small>PDF, PNG or JPG. This sprint records the filename; secure document storage remains a future task.</small></label>
    {!eligibleOrders.length ? <div className="full validation-message">No completed, uninvoiced work orders are available for {practice}.</div> : null}
    <div className="full form-note">Submission writes an auditable invoice to the same Firebase project used for authentication. Finance export becomes available only after GAP approval.</div>
  </div>;
}
